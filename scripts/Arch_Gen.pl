...
COMMAND_LINE: {
	if (@ARGV < 5 ) {die "Five arguments are required: house_types regions set_name simulation_time-step_(minutes) BCD_mode\n";};	# check for proper argument count
    ...
    $iBCD_Mode = shift (@ARGV);
    if(($iBCD_Mode<0) || ($iBCD_Mode>1)) {die "Invalid BCD mode; 0 = Measured Profiles. 1 = 2015 NBC profiles\n";}
...
# -----------------------------------------------
# Develop code compliance keys
# -----------------------------------------------
my $hCodeComply = &key_XML_readin('../keys/NBC_Keys.xml', [1]);	# readin the HVAC cross ref
foreach my $region (values (%{$regions})) {	# Multithread for each region
    for(my $i=1;$i<=$hCodeComply->{'Cities'}->{"_$region"}->{'n'};$i++) {
        my $sCity=$hCodeComply->{'Cities'}->{"_$region"}->{"City_$i"};
        print "Archetypes for $sCity in region $region will be generated\n";
        push(@{$hLocations->{$region}},$sCity);
    };
};

# -----------------------------------------------
# Add the construction files including the archetype constructions
# -----------------------------------------------
($mat_data, $con_data, $optic_data) = &AddArchetypedatabase_XML($mat_data, $con_data, $optic_data);	# construct the databases and leave the information loaded in the variables for use in house generation
...
# -----------------------------------------------	
# Load in the measured DHW profiles if required
my $MeasuredFlow;
if(not $iBCD_Mode) {
    my $DHW_XML = "../bcd/DHW_AL_source/Measured_DHW_Files.xml";
    $MeasuredFlow = XMLin($DHW_XML, keyattr => [], forcearray => 0);
...
my $BCD_al_ann={}; 
# Load the new electrical profiles
foreach my $sLoad (qw(Dryer Other Stove)) {
    open(my $fid, "<", "../bcd/NewAL/$sLoad"."_Demands_1min.csv") or die "Can't open < $sLoad Demands_1min.csv: $!";
    my @sAllData = <$fid>;
    close $fid;
    my $sThisHead=shift @sAllData;
    chomp $sThisHead;
    my @sHead1 = split /,/, $sThisHead;
    $sThisHead = shift @sAllData;
    chomp $sThisHead;
    my @sHead2 = split /,/, $sThisHead;
    my $iProfs = $sHead1[1]/1.0;
    for(my $i=1;$i<=$iProfs;$i++) {
        $BCD_al_ann->{$sLoad}->{"$i"}->{'GJ_Yr'}=$sHead2[$i]/1.0;
    };
    
    foreach my $sLine (@sAllData) {
        chomp $sLine;
        my @sThisLine = split /,/,$sLine;
        for(my $i=1;$i<=$iProfs;$i++) {
            chomp $sThisLine[$i];
            push(@{$BCD_al_ann->{$sLoad}->{"$i"}->{'profile'}},$sThisLine[$i]*1000); # Convert from kW to W
        };
    };
};
...
MULTI_THREAD: {
	...
	my $input_path = '../CSDDRD/CSDDRD_DHW_AL_BCD_MULT'; # TODO: Determine if we need this
...
# 				else {&error_msg ('Bad foundation determination', $coordinates);};
                ...
                #my $ceiling_dominant;
				if (($CSDDRD->{'roof_type'} != 1) && ($CSDDRD->{'roof_type'} != 5)) {	# set attic zone indicator unless flat ceiling is type "N/A" or "flat"
					$zones->{'name->num'}->{'attic'} = keys(%{$zones->{'name->num'}}) + 1;
					
					# Record the above/below zone info
					$zones = &lower_and_upper_zone($zones, 'main_' . $high_level, 'attic');
                    #$ceiling_dominant = 'sloped';
				}
				
				# CEILING TYPE ERROR
				elsif (($CSDDRD->{'roof_type'} < 1) || ($CSDDRD->{'roof_type'} > 6)) {
# 					&error_msg ('Bad flat roof type', $coordinates);
					&die_msg ('ZONE PRESENCE: Bad flat roof type (<1 or >6)', $CSDDRD->{'roof_type'}, $coordinates);
				}
						...
						foreach my $zone (keys (%{$zones->{'name->num'}})) {
							...
							# check for walkout basements and if so create a tmc file if the window area matches that side
								...
								CHECK_BSMT_TMC: foreach my $surface (@sides) {
									...
									if ($CSDDRD->{'wndw_area_' . $surface} > 0.5 && ($record_indc->{'foundation'} =~ $surface ||  $CSDDRD->{"bsmt_$surface"."_wndw"})) {
										&copy_template($zone, $ext, $hse_file, $coordinates);
...
# 						print GEN_SUMMARY "\tDoor\@[$index] width/height reversed: $coordinates\n";	# print a comment about it
				...
				foreach my $zone (keys(%{$zones->{'name->num'}})) {
					...
					foreach my $surface (@sides) { 
						...
						elsif ($zone eq 'bsmt' && ($record_indc->{'foundation'} =~ $surface || $CSDDRD->{"bsmt_$surface"."_wndw"})) {
							# the available surface area on that side is the side width minus door and three margins multiplied by the height minus two margins
				...
				# cycle through and check the surface area to window size
				foreach my $surface (@sides) {
			...
			GEO_SURFACES: {
				foreach my $zone (@{$zones->{'num_order'}}) {
					...
					# Zone must be an attic
						...
						if (($CSDDRD->{'roof_type'} == 2) || ($CSDDRD->{'attachment_type'} == 4)) {	
						...
						elsif ($CSDDRD->{'roof_type'} == 3) {	# Hip roof
							my $peak_y_minus;
							my $peak_y_plus;
							my $peak_x_minus;
							my $peak_x_plus;
					...
					foreach $surface (@sides) {
						...
						if ($zone =~ /^main_\d$/ || (($zone eq 'bsmt' && ($record_indc->{'foundation'} =~ $surface || $CSDDRD->{"bsmt_$surface"."_wndw"})) && $CSDDRD->{'wndw_area_' . $surface} > 0)) {
                            ...
                            my $Dummy = ($record_indc->{'wndw'}->{$zone}->{'available-SA'}->{$surface} * ($z2 - $z1) ** 2) ** 0.5;
                            if($Dummy == 0) {
                                print Dumper $record_indc->{'wndw'}->{$zone}->{'available-SA'}->{$surface};
                                print Dumper $zone;
                                print Dumper $z2;
                                print Dumper $z1;
                                print Dumper $surface;
                                print Dumper $wndw_area;
                            };
			...
			GEO_ZONING: {
				...
				foreach my $zone (@{$zones->{'vert_order'}}) {	# Do in vertical order because the each levels floor (e.g. main_1) will reverse the previous levels ceiling (e.g. bsmt)
					...
					if ($zone eq 'bsmt') {	# build the floor, ceiling, and sides surfaces and attributes for the bsmt
						...
						FLOOR_BSMT: {
							...
							if ($CSDDRD->{$field_name . '_coverage'} == 5) {
                                ...
                                if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                    $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'below_grade_with_HRV'}->{'Floors_heated'}->{$sThisClimateZone});
                                } else {
                                    $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'below_grade_no_HRV'}->{'Floors_heated'}->{$sThisClimateZone});
                                };
                                con_surf_conn($fThisRSI, $zone, $surface, $zones, $record_indc, $issues, $coordinates);
							}
						...
						SIDES_BSMT: foreach my $surface (@sides) {
							...
							SIDES_ONLY_BSMT: {
								...
								# check to see if we are a WALKOUT side
                                    ...
                                    my $fThisRSI;
                                    if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                        $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_with_HRV'}->{'Walls'}->{$sThisClimateZone});
                                    } else {
                                        $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_no_HRV'}->{'Walls'}->{$sThisClimateZone});
                                    };
                                    con_surf_conn($fThisRSI, $zone, $surface, $zones, $record_indc, $issues, $coordinates);
								...
								# otherwise it is a STANDARD BASEMENT WALL
									...
									if ($CSDDRD->{'bsmt_exterior_insul_coverage'} =~ /^[2-4]$/ || $CSDDRD->{'bsmt_interior_insul_coverage'} =~ /^[2-4]$/) {
                                        ...
                                        my $fThisRSI;
                                        if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                            $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'below_grade_with_HRV'}->{'Walls'}->{$sThisClimateZone});
                                        } else {
                                            $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'below_grade_no_HRV'}->{'Walls'}->{$sThisClimateZone});
                                        };
										
										# initialize the code because we want to indicate the defined type or let it be replaced by the top insul code
										$con->{'code'} = '';
                                        
                                        # While not critical, if exterior and interior insulation is present, split the RSI between the two
                                        # BASESIMP is the primary routine that determines the flux in/out of the basement
                                        my $fScalar;
                                        if (($CSDDRD->{'bsmt_exterior_insul_coverage'} =~ /^[2-4]$/) && ($CSDDRD->{'bsmt_interior_insul_coverage'} =~ /^[2-4]$/)) {
                                            $fScalar=0.5;
                                        } else {
							...
							WNDW_DOOR_BSMT: {
                                ...
                                # Generate the window code for this climate zone
								if (defined ($record_indc->{$zone}->{'surfaces'}->{$surface . '-aper'})) {
                                    my ($iClimZone) = $sThisClimateZone =~ m/Zone_(.*)$/;
                                    my $iGlzComp = $hCodeComply->{'Glazing'}->{"$sThisClimateZone"};
                                    my $wndw_code = "Z_$iClimZone"."_$iGlzComp";
									...
									# and the frame
                                    my $fr_code = $con_data->{$con->{'name'}}->{'frame'};
								...
								if (defined ($record_indc->{$zone}->{'surfaces'}->{$surface . '-door'})) {
                                    ...
                                    my $fThisRSI = 1.0/$hCodeComply->{'Envelope_RSI'}->{'Fene_Door'}->{'Max_U_Value'}->{$sThisClimateZone};
									con_surf_conn($fThisRSI, $zone, $surface . '-door', $zones, $record_indc, $issues, $coordinates);
								};
							};
						};
						...
						(my $insul_RSI, $issues) = check_range("%.1f", $fThisRSI, 0, 9, 'BASESIMP Insul RSI', $coordinates, $issues); # set the insul value to the larger of interior/exterior insulation of basement
						...
						CEILING_CRAWL: {
                            ...
                            my $fThisRSI;
                            if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_with_HRV'}->{'Floors_unheated'}->{$sThisClimateZone});
                            } else {
                                $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_no_HRV'}->{'Floors_unheated'}->{$sThisClimateZone});
                            };
							con_surf_conn($fThisRSI, $zone, $surface, $zones, $record_indc, $issues, $coordinates);
						};
						...
						FLOOR_MAIN: {
							...
							# There is no zone below, so check to see if it is SLAB ON GRADE
								...
								if ($CSDDRD->{$field_name . '_coverage'} == 4) { # Edge insulated
                                    ...
                                    my $fThisRSI;
                                    if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                        $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_with_HRV'}->{'slab'}->{$sThisClimateZone});
                                    } else {
                                        $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_no_HRV'}->{'slab'}->{$sThisClimateZone});
                                    };
									con_surf_conn($fThisRSI, $zone, $surface, $zones, $record_indc, $issues, $coordinates);
								}
						...
						CEILING_MAIN: {
							my $surface = 'ceiling';
							my $con = \%{$record_indc->{$zone}->{'surfaces'}->{$surface}->{'construction'}};
                            if(($level == $high_level) && ($CSDDRD->{'roof_type'} == 5)) {
                                facing('EXTERIOR', $zone, $surface, $zones, $record_indc, $coordinates);
                            } else {
                                facing('ANOTHER', $zone, $surface, $zones, $record_indc, $coordinates);
                            };
							...
							if ($level == $high_level) {
                                my $field_name = 'ceiling';

								if($CSDDRD->{'roof_type'} != 5) {
                                    $con->{'name'} = 'M->A_or_R';
								} else {
                                    $field_name = 'ceiling_exposed';
                                    $con->{'name'} = 'M_ceil_exp';
                                };
                                ...
                                my $sCeilingType;
                                if($CSDDRD->{'roof_type'} != 5) {
                                    $sCeilingType='Ceilings_below_attics';
                                } else {
                                    $sCeilingType='Flat_Roof';
                                };
                                if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                    $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_with_HRV'}->{$sCeilingType}->{$sThisClimateZone});
                                } else {
                                    $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_no_HRV'}->{$sCeilingType}->{$sThisClimateZone});
                                };
								con_surf_conn($fThisRSI, $zone, $surface, $zones, $record_indc, $issues, $coordinates);
							}
						...
						CEILING_EXPOSED_MAIN: {
							...
							if (defined ($record_indc->{$zone}->{'surfaces'}->{$surface})) {
                                ...
                                my $fThisRSI;
                                if ($CSDDRD->{'vent_equip_type'} == 2 || $CSDDRD->{'vent_equip_type'} == 5) {	# HRV is present
                                    $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_with_HRV'}->{'Flat_Roof'}->{$sThisClimateZone});
                                } else {
                                    $fThisRSI = ($hCodeComply->{'Envelope_RSI'}->{'above_grade_no_HRV'}->{'Flat_Roof'}->{$sThisClimateZone});
                                };
								con_surf_conn($fThisRSI, $zone, $surface, $zones, $record_indc, $issues, $coordinates);
							};
						};
						...
						if ($level == 1 && $record_indc->{'foundation'} eq 'slab') {
							...
							(my $insul_RSI, $issues) = check_range("%.1f",$fThisRSI, 0, 9, 'BASESIMP Insul RSI', $coordinates, $issues);
							&replace ($hse_file->{"$zone.bsm"}, "#RSI", 1, 1, "%s\n", "$insul_RSI");
						};
					}
...
#print $con->{'type'};
                                ...
                                if($surface =~ m/(floor-exposed)/) {
                                    push (@{$em_abs->{'abs'}->{'outside'}},$hCodeComply->{'Solar_abs'}->{'exposed_floor'});
                                } elsif($zone =~ m/^roof$|^attic$/ && $surface_basic !~ m/^floor$/) {
                                    push (@{$em_abs->{'abs'}->{'outside'}},$hCodeComply->{'Solar_abs'}->{'roof'});
                                } elsif($surface_basic !~ m/^floor$|^ceiling$/ && $other !~ m/aper$/) {
                                    push (@{$em_abs->{'abs'}->{'outside'}},$hCodeComply->{'Solar_abs'}->{'walls'});
                                } else {
...
# 									};
	...
	# 									};
			...
			BCD: {
                ...
                my @TotalCook; # Array to hold the annual power draw of range and oven (base electric) [W]
                my @TotalDry; # Array to hold the annual power draw of dryer (base electric) [W]
				...
				my @bcd_fields = ('AL-Other_GJpY', 'AL-Stove_GJpY', 'AL-Dryer_GJpY','DHW_LpY');
                
				foreach my $field (@bcd_fields) {
					$bcd_match->{$field} = {'filename' => 'big-example', 'difference' => 1e9, 'index' => -1, 'source' => ''};
				};
                ...
                if(not $iBCD_Mode) {
                    ...
                    my $ThisDHWann = $CSDDRD->{'DHW_LpY'}; # Litres per year
                    ...
                    # Using the NBC profiles
                    my $sBCDfile = $hCodeComply->{'DHW'}->{'source'} . "_$hCodeComply->{'DHW'}->{'daily_consump'}" . "_L_Day_DHW_$time_step"."_min.bcd";
                    &replace ($hse_file->{'cfg'}, "#BCD", 1, 1, "%s\n", "*bcd ../../../bcd/$sBCDfile");	# boundary condition path
                    
                    # Set the multipliers
                    foreach my $field (@bcd_fields) {
                        $BCD_characteristics->{$CSDDRD->{'file_name'}}->{$field}->{'multiplier'} = 1.0;
                        $BCD_characteristics->{$CSDDRD->{'file_name'}}->{'DHW_LpY'}->{'unmultiplied'} = $hCodeComply->{'DHW'}->{'daily_consump'}*365.0;
                    };
                ...
                # MATCH_AL
                if(not $iBCD_Mode){
                    foreach my $field (@bcd_fields) {	# the AL fields
                        my $bcd;
                        if ($field eq 'AL-Other_GJpY') {
                            $bcd="Other";
                        } elsif ($field eq "AL-Dryer_GJpY") {
                            $bcd="Dryer";
                        } elsif ($field eq "AL-Stove_GJpY") {
                            $bcd="Stove";
                        } elsif ($field eq "DHW_LpY") {
                            next;  
                        } else {&die_msg ("BCD ISSUE: there is no search defined for this field", $field, $coordinates);};
                        
                        my $hHashRef = $BCD_al_ann->{$bcd};
                        FIND_AL_REC: foreach my $ALrec (sort { $hHashRef->{$a}->{'GJ_Yr'} <=> $hHashRef->{$b}->{'GJ_Yr'} } keys %{$hHashRef}) {
                            my $difference = abs $CSDDRD->{$field} - $BCD_al_ann->{$bcd}->{"$ALrec"}->{'GJ_Yr'};
                            if($difference < $bcd_match->{$field}->{'difference'}) {
                                # update the value
                                $bcd_match->{$field}->{'difference'} = $difference;
                                $bcd_match->{$field}->{'filename'} = $bcd."_Demands_1min.csv Column $ALrec";
                                $bcd_match->{$field}->{'index'} = $ALrec;
                                
                                # Determine the DHW multiplier
                                $BCD_characteristics->{$CSDDRD->{'file_name'}}->{$field}->{'multiplier'} = $CSDDRD->{$field}/$BCD_al_ann->{$bcd}->{"$ALrec"}->{'GJ_Yr'};
                            } elsif($difference == $bcd_match->{$field}->{'difference'}) {
                                next FIND_AL_REC;
                            } else {
                                last FIND_AL_REC;
                            };
                        ...
                        # Load the profiles for printing
                        @TotalOther = @{$BCD_al_ann->{'Other'}->{$bcd_match->{$field}->{'index'}}->{'profile'}};
                        @TotalDry = @{$BCD_al_ann->{'Dryer'}->{$bcd_match->{$field}->{'index'}}->{'profile'}};
                        @TotalCook = @{$BCD_al_ann->{'Stove'}->{$bcd_match->{$field}->{'index'}}->{'profile'}};
                ...
                if(not $iBCD_Mode) { 
                    ...
                    &insert($hse_file->{'bcd'},"#END_NOTES",1,0,0,"%s\n","# Stove Annual Energy: $StoveE kWh (unmultiplied)");
                    &insert($hse_file->{'bcd'},"#END_NOTES",1,0,0,"%s\n","# Dryer Annual Energy: $DryerE kWh (unmultiplied)");
                    &insert($hse_file->{'bcd'},"#END_NOTES",1,0,0,"%s\n","# Other Energy: $OtherE kWh (unmultiplied)");
                    ...
                    my $sThisDwell=$CSDDRD->{'file_name_orig'};
                    &replace ($hse_file->{'cfg'}, "#BCD", 1, 1, "%s\n", "*bcd ../../BCD/$sThisDwell.bcd");	# boundary condition path
                }; # END WRITE_BCD
				...
				AL: {
					...
					$mult->{'AL-Dryer'} = $BCD_characteristics->{$CSDDRD->{'file_name'}}->{'AL-Dryer_GJpY'}->{'multiplier'};
					# stove and other mult = AL-Stove-Other / (BCD-Stove-Other)
					$mult->{'ALStove'} = $BCD_characteristics->{$CSDDRD->{'file_name'}}->{'AL-Stove_GJpY'}->{'multiplier'};
					# note that the AL-Other is the same multiplier as AL-Stove
					$mult->{'ALOtherElectric'} = $BCD_characteristics->{$CSDDRD->{'file_name'}}->{'AL-Other_GJpY'}->{'multiplier'};
...
# 					if ($CSDDRD->{'dryer_fuel_used'} == 1) {$mult->{'AL-Dryer'}  = $mult->{'AL-Dryer'} * 1.10};
					...
					my $num_adults = $CSDDRD->{'Num_of_Adults'};
					my $num_childs = $CSDDRD->{'Num_of_Children'};
					my $emp_ratio = $CSDDRD->{'Employment_Ratio'};
                    if($iBCD_Mode) {
                        $num_adults=0;
                        $num_childs=0;
                        $emp_ratio=0;
	...
	# 			-----------------------------------------------
				...
				DHW: {
					...
					if ($CSDDRD->{'DHW_energy_src'} == 9) {	# DHW is not available, so comment the *dhw line in the cfg file
						...
						&replace ($hse_file->{"dhw"}, "#BCD_MULTIPLIER", 1, 1, "%.2f # %s\n", $multiplier, "House annual draw = $CSDDRD->{'DHW_LpY'} LpY; Source profile annual draw = $BCD_characteristics->{$CSDDRD->{'file_name'}}->{'DHW_LpY'}->{'unmultiplied'} LpY");	# DHW multiplier
...
# 			-----------------------------------------------
			...
			HVAC: {
                ...
                # determine the heating setpoint
                my $heat_T = $CSDDRD->{'main_floor_heating_temp'};
                # determine the cooling setpoint
                my $cool_T = $CSDDRD->{'main_floor_cooling_temp'};
...
# 					print "system $system; systems $systems[$system]; equip $equip_key{$systems[$system]}\n";
				...
				foreach my $zone (@{$zones->{'num_order'}}) {
					...
					# Central heating systems require a slave/master control approach
						...
						unless (($zone =~ /^main_1$/) || (($zone =~ /^bsmt$/) && ($CSDDRD->{'bsmt_seperate_thermostat'}))) {
						...
						# The main_1 zone is the master controller, so simply set it to a basic five season
                            ...
                            my $fThisHeat=$heat_T;
                            my $fThisCool=$cool_T;
                            if($zone =~ /^bsmt$/) {$fThisHeat=$CSDDRD->{'bsmt_heated'};}
							&insert ($hse_file->{'ctl'}, '#END_FUNCTION_DATA', 1, 0, 0, "%s", &basic_5_season($zones->{'name->num'}->{$zone}, $ctl_params->{'heat_cap'} * 1000, $ctl_params->{'cool_cap'} * 1000, $record_indc->{$zone}->{'volume'} / $record_indc->{'vol_conditioned'},$fThisHeat,$fThisCool));
						};
					}
					
					# The remaining heat type is distributed, so each zone gets a basic controller and the capacity is adjusted based on volume
					else {
						&insert ($hse_file->{'ctl'}, '#END_FUNCTION_DATA', 1, 0, 0, "%s", &basic_5_season($zones->{'name->num'}->{$zone}, $ctl_params->{'heat_cap'} * 1000, $ctl_params->{'cool_cap'} * 1000, $record_indc->{$zone}->{'volume'} / $record_indc->{'vol_conditioned'},$heat_T,$cool_T));
					};
				};
...
# 				};
			...
			AIM: {
                ...
                &replace ($hse_file->{'aim'}, "#BLOWER_DOOR", 1, 1, "%s\n", "1 2 $hCodeComply->{'ACH'} 10");	# Airtightness rating, use ACH50 only (as selected in HOT2XP)
				...
				my $rural_suburb_urban = $CSDDRD->{'Rural_Suburb_Urban'};
...
# 				&replace ($hse_file->{'aim'}, '#WINDOW_CONTROL', 1, 1, "%s\n", "1 $zones->{'name->num'}->{'main_1'} 25 21 1 21");
			...
			FILE_PRINTOUT: {
                ...
                my $BDCfolder = "../$hse_type$set_name/BCD";
                if(not $iBCD_Mode) {if (not (-d $BDCfolder)) {mkpath ($BDCfolder)};}
				...
				foreach my $ext (keys %{$hse_file}) {	# go through each extention inclusive of the zones for this particular record
                    ...
                    if($ext =~ /bcd/) { # BCD Files stored in central folder
                        $file = "../$hse_type$set_name/BCD/$CSDDRD->{'file_name_orig'}.";
                    } else {
